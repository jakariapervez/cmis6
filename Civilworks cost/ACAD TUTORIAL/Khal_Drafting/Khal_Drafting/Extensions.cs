...
namespace Khal_Deafting
{
    ...
    public class Commands
    {
        ...
        public void CreateLayout(string sheetName,DrawingSheet ds)
        {
            var doc = Application.DocumentManager.MdiActiveDocument;
            ...
            {
                // Create and select a new layout tab
                var id = LayoutManager.Current.CreateAndMakeLayoutCurrent(sheetName);
                // Open the created layout
                var lay = (Layout)tr.GetObject(id, OpenMode.ForWrite);
                // Make some settings on the layout and get its extents
                lay.SetPlotSettings(
...

         {
             ...
             if (ValidDbExtents(ds.sheetExtents.MinPoint, ds.sheetExtents.MaxPoint))

             {

               //  vp.FitContentToViewport(new Extents3d(ds.sheetExtents.MinPoint, ds.sheetExtents.MinPoint), 0.9);
