...

{
...

    {
        ...
        */
        ...
        public string DrawingNo { get; set; }
        public string Drawing_sheet_no { get; set; }
        public Point2d TopLeft { get; set; }
        public int width { get; set; }
        public int height { get; set; }
        public Extents3d sheetExtents { get; set; }
        public DrawingSheet()
        {
            // Labels
        ...
        public void DrawTitleBlock2(int width)
        {
            ...
            using (Transaction trans = doc.TransactionManager.StartTransaction())
            {
                ...
                this.sheetExtents = pl.GeometricExtents;
        ...
        public void DrawTitleBlock3()
        {
            ...
            using (Transaction trans = doc.TransactionManager.StartTransaction())
            {
                ...
                trans.AddNewlyCreatedDBObject(pl, true);
                this.sheetExtents = pl.GeometricExtents;
                //Draw the border for sheet title
                pl = new Polyline();
                pl.AddVertexAt(0, new Point2d(x0+0.89*w, y0), 0, 0, 0);
                pl.AddVertexAt(1, new Point2d(x0 + w, y0), 0, 0, 0);
                pl.AddVertexAt(2, new Point2d(x0 + w, y0-0.03*w), 0, 0, 0);
                pl.AddVertexAt(3, new Point2d(x0 + 0.89 * w, y0 - 0.03 * w), 0, 0, 0);
                pl.Closed = true;
                btr.AppendEntity(pl);
                trans.AddNewlyCreatedDBObject(pl, true);
                //writng sheet no
               // string[] drawingno_elements =this.DrawingNo.Split('-');
                MText txtSheetNo = new MText();
                txtSheetNo.Contents = "Sheet No:" + this.Drawing_sheet_no;
                txtSheetNo.TextHeight = width * 0.008;
                double xCoord = x0 + 0.945 * width;
                double yCoord = y0 - 0.015 * w;
                Point3d insPt = new Point3d(xCoord, yCoord, 0);
                txtSheetNo.Location = insPt;
                txtSheetNo.Attachment = AttachmentPoint.MiddleCenter;
                btr.AppendEntity(txtSheetNo);
                trans.AddNewlyCreatedDBObject(txtSheetNo, true);
        ...
        public Point2d DrawingOriginA()
        {
            ...
            double y0 = this.TopLeft.Y;
            double w = this.width;
            Point2d a = new Point2d(x0 + 0.1 * w, y0 - 0.05 * w);
            return a;
        }
        public Point2d DrawingOriginB()
        {
            double x0 = this.TopLeft.X;
            double y0 = this.TopLeft.Y;
            double w = this.width;
            Point2d a = new Point2d(x0 + 0.6 * w, y0 - 0.05 * w);
            return a;
        }
        public Point2d DrawingOriginC()
        {
            double x0 = this.TopLeft.X;
            double y0 = this.TopLeft.Y;
            double w = this.width;
            Point2d a = new Point2d(x0 + 0.1 * w, y0 - (0.55 * w / Math.Sqrt(2)));
            return a;
        }
