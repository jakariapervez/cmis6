...

    {
        ...
        public void DrawTitleBlock2( int width)
        {
            ...
            using (Transaction trans=doc.TransactionManager.StartTransaction()) 
            {
                ...
                title_block_border.AddVertexAt(1, new Point2d(x0 + 0.7 * width, y0 - height*0.77), 0, 0, 0);
                title_block_border.AddVertexAt(2, new Point2d(x0 +  width, y0 - height * 0.77), 0, 0, 0);
                btr.AppendEntity(title_block_border);
                trans.AddNewlyCreatedDBObject(title_block_border, true);
                //First Horizonal Line
                Polyline L1 = new Polyline();
                L1.AddVertexAt(0, new Point2d(x0+width*0.7,y0-height*0.785), 0, 0, 0);
